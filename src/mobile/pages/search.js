import { mobileCatalogData, isUserBlacklisted, isOfferBlacklisted } from '../../core/state.js';
import { extractUserIdFromMobileData, getMobileOfferId, fetchMobileCatalogIfNeeded } from '../parser.js';
...
export function updateMobileOfferState(offerElement, offerInfo) {
...
export async function processMobileSearchPage() {
  const hiddenContainer = createHiddenContainer();
  ...
  if (hiddenContainer) {
    ...
    for (const clonedOffer of clonedOffers) {
      ...
      updateMobileOfferState(clonedOffer, { offerId, userId });
    }
  }
